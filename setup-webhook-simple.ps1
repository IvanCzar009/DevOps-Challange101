...
try {
    ...
    $response = Invoke-WebRequest -Uri $githubApiUrl -Method GET -UseBasicParsing
    $existingHooks = $response.Content | ConvertFrom-Json
    
    # Check if webhook already exists
    $webhookExists = $false
    foreach ($hook in $existingHooks) {
        if ($hook.config.url -eq $webhookUrl) {
            $webhookExists = $true
            Write-Host "✅ Webhook already exists!" -ForegroundColor Green
            Write-Host "   Webhook ID: $($hook.id)" -ForegroundColor White
            Write-Host "   URL: $($hook.config.url)" -ForegroundColor White
            Write-Host "   Active: $($hook.active)" -ForegroundColor White
            $success = $true
            break
        }
    }
    ...
    if (-not $webhookExists) {
        ...
        $createResponse = Invoke-WebRequest -Uri $githubApiUrl -Method POST -Body $webhookPayload -ContentType "application/json" -UseBasicParsing
        $newWebhook = $createResponse.Content | ConvertFrom-Json
